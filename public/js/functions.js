...
function performSearch(
  ...
  var interval1 = setInterval(function() {
    if (lon != "undefined" && lat != "undefined") {
      ...
      queryUrl =
        "https://tripadvisor1.p.rapidapi.com/restaurants/list-by-latlng?limit=100&currency=USD&distance=" +
        distance +
        "&lunit=mi&lang=en_US&min_rating=" +
        rating +
        "&latitude=" +
        lat +
        "&longitude=" +
  ...
  if (result.length == 0) {
    ...
    for (i in result) {
      ...
      (span = createStarRating(result[i].rating)),
        (div2 = $("<div>")
          .attr("class", "result")
          .text("Address: " + result[i].address)),
        (div3 = $("<div>")
          .attr("class", "result")
          .text("Cuisines: " + cuisinesStr)),
        (div4 = $("<div>")
          .attr("class", "result")
          .text("Phone: " + result[i].phone)),
        (div5 = $("<div>")
          .attr("class", "result")
          .text("Price Range: " + result[i].price_level)),
        (div6 = $("<div>")
          .attr("class", "result")
          .text("Open now: " + result[i].open_now_text)),
        (div7 = $("<a>")
      ...
      var img2 = $("<img>")
        .attr("src", "assets/images/share.png")
...
function displayDistance() {
  ...
  var interval1 = setInterval(function() {
    if (userLon != "undefined" && userLat != "undefined") {
      ...
      console.log(
        "place location: " + selected.latitude + " " + selected.longitude
      );
...
function retrieveReviews(location_id, review_num) {
  ...
  var queryUrl =
    "https://tripadvisor1.p.rapidapi.com/reviews/list?limit=" +
    review_num +
    "&currency=USD&lang=en_US&location_id=" +
    location_id;
  $.ajax(getAjaxSetting(queryUrl)).done(function(response) {
    console.log(response);
    response = response.data;
    $("#review_modal_title").html(
      selected.name + " " + createStarRating(selected.rating)
    );
    ...
    for (var i = 0; i < response.length; i++) {
      ...
      date = $("<div>")
        .attr("class", "published_date")
        .html(date + createStarRating(response[i].rating));
      var title = $("<div>")
        .attr("class", "review_title")
      ...
      $("#review_container").append(
        $("<div>")
          .attr("class", "each_review")
          .append(date, title, text)
      );
    }
  });
}
